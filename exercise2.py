__author__ = 'dortega'


import sys


##Many Time Pad

##Let us see what goes wrong when a stream cipher key is used more than once. Below are eleven hex-encoded ciphertexts that are the result of encrypting eleven plaintexts with a stream cipher, all with the same stream cipher key. Your goal is to decrypt the last ciphertext, and submit the secret message within it as solution.

##Hint: XOR the ciphertexts together, and consider what happens when a space is XORed with a character in [a-zA-Z].

##https://gist.github.com/EarlGray/3706892


print 'Week 1 - Excercise 2'

MSGS = [None] * 11

ciphertext1 = ''
ciphertext2 = '234c02ecbbfbafa3ed18510abd11fa724fcda2018a1a8342cf064bbde548b12b07df44ba7191d9606ef4081ffde5ad46a5069d9f7f543bedb9c861bf29c7e205132eda9382b0bc2c5c4b45f919cf3a9f1cb74151f6d551f4480c82b2cb24cc5b028aa76eb7b4ab24171ab3cdadb8356f'
ciphertext3 = '32510ba9a7b2bba9b8005d43a304b5714cc0bb0c8a34884dd91304b8ad40b62b07df44ba6e9d8a2368e51d04e0e7b207b70b9b8261112bacb6c866a232dfe257527dc29398f5f3251a0d47e503c66e935de81230b59b7afb5f41afa8d661cb'
ciphertext4 = ''
ciphertext5 = '3f561ba9adb4b6ebec54424ba317b564418fac0dd35f8c08d31a1fe9e24fe56808c213f17c81d9607cee021dafe1e001b21ade877a5e68bea88d61b93ac5ee0d562e8e9582f5ef375f0a4ae20ed86e935de81230b59b73fb4302cd95d770c65b40aaa065f2a5e33a5a0bb5dcaba43722130f042f8ec85b7c2070'
ciphertext6 = ''
ciphertext7 = ''
ciphertext8 = '315c4eeaa8b5f8bffd11155ea506b56041c6a00c8a08854dd21a4bbde54ce56801d943ba708b8a3574f40c00fff9e00fa1439fd0654327a3bfc860b92f89ee04132ecb9298f5fd2d5e4b45e40ecc3b9d59e9417df7c95bba410e9aa2ca24c5474da2f276baa3ac325918b2daada43d6712150441c2e04f6565517f317da9d3'
ciphertext9 = ''
ciphertext10 = '466d06ece998b7a2fb1d464fed2ced7641ddaa3cc31c9941cf110abbf409ed39598005b3399ccfafb61d0315fca0a314be138a9f32503bedac8067f03adbf3575c3b8edc9ba7f537530541ab0f9f3cd04ff50d66f1d559ba520e89a2cb2a83'

targetCiphertext = '32510ba9babebbbefd001547a810e67149caee11d945cd7fc81a05e9f85aac650e9052ba6a8cd8257bf14d13e6f0a803b54fde9e77472dbff89d71b57bddef121336cb85ccb8f3315f4b52e301d16e9f52f904'

MSGS[0] = ciphertext1;
MSGS[1] = ciphertext2;
MSGS[2] = ciphertext3;
MSGS[3] = ciphertext4;
MSGS[4] = ciphertext5;
MSGS[5] = ciphertext6;
MSGS[6] = ciphertext7;
MSGS[7] = ciphertext8;
MSGS[8] = ciphertext9;
MSGS[9] = ciphertext10;
MSGS[10] = targetCiphertext;


key1 = "66396e89c9"
key2 = "66396e89c9000d800000000000063000000af000000000000a06e00c900000000690000db000000001a000000000000000000f0f00000480000000200000000770000000000009c000000260000000000009a000098000000006100c70000000000000000000000000000db000000000"
key3 = "66396e89c9dbd8cb9874352acd6300102eafce78aa7fed28a06e6bc98d29c51969b025c919f8aa401a9c6d708f80c066c763fef0123148cdd8e802d05ba98777335daefcecd59c433a6b268b60bf4ef03c9a611098bb009a3161edc70004000000cf000000000000006edba8c2000000"
key4 = "66396e89c9dbd8cc9874352acd6300102eafce78aa7fed28a06e6bc98d29c51969b025c919f8aa401a9c6d708f80c066c763fef0123148cdd8e802d05ba98777335daefcecd59c433a6b268b60bf4ef03c9a611098bb009a3161edc70004000000cf000000000000006edba8c2000000"

def strxor(a, b):     # xor two strings of different lengths
    if len(a) > len(b):
        return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(a[:len(b)], b)])
    else:
        return "".join([chr(ord(x) ^ ord(y)) for (x, y) in zip(a, b[:len(a)])])


def random(size=16):
    return open("/dev/urandom").read(size)


def encrypt(key, msg):
    c = strxor(key, msg)
    print
    print c.encode('hex')
    return c

def decrypt(key, ct):
    m = strxor(key, ct)
    return m


def main():
    key = random(1024)
    ciphertexts = [encrypt(key, msg) for msg in MSGS]


def getCol(lista,pos):
    result = ""
    for i in range(len(lista)):
        if pos < len(lista[i]):
            result += lista[i][pos]

    return result


def isALetter(letter):
    #return ord(letter)>63
    return letter.isalpha()

def detectSpaces(ct,MSGS):
    lista = []
    for ct2 in MSGS:
        result = ""
        if ct != ct2:
            s1 = strxor(ct.decode("hex"),ct2.decode("hex"))

            for letter in s1:
                if isALetter(letter):
                    result += "_"
                else:
                    result += "X"

            lista.append(result)

    spaces = ""
    for i in range(len(lista[0])):
        aux = getCol(lista, i)
        counter = aux.count('_')

        if counter > 6:
            spaces += "_"
        else:
            spaces += "Y"

    return spaces


def break1(ct,spaces):
    toProcess = spaces
    toProcess = toProcess.replace('Y','00')
    toProcess = toProcess.replace('_','20')

    lon = len(ct)
    if len(toProcess) < lon:
        lon = len(toProcess)

    lon /=2
    key = ""
    for i in range(lon):
        a = ct[2*i]+ct[2*i+1]
        b = toProcess[2*i]+toProcess[2*i+1]
        if b == "20":
            key += strxor(a.decode("hex"),'20'.decode("hex")).encode("hex")
        else:
            key += "00"
    return key





def showPartialKey(partialKey):

    result = partialKey.replace("00","2d")
    return result.decode("hex")


def joinPartialKeys(partialKeys):

    result = ""
    for i in range(len(partialKeys[0])):
        aux = getCol(partialKeys, i)
        subkey = "\0"
        for letter in aux:
            if letter != '\0':
                subkey = letter

        result += subkey
    return result







partialKeys = []
for ct in MSGS:
    print decrypt(ct.decode("hex"),key4.decode("hex"))
    spaces = detectSpaces(ct,MSGS)
    key = break1(ct,spaces)
    partialKeys.append(key.decode("hex"))
    #print showPartialKey(key)
    ##print decrypt(ct.decode("hex"),key.decode("hex"))


yrchKey = joinPartialKeys(partialKeys).encode("hex")

print "A ver bonito.."
for ct in MSGS:
    print decrypt(ct.decode("hex"),yrchKey.decode("hex"))





print strxor("AA".decode("hex"),"66".decode("hex")).encode("hex")

#for ct in MSGS:
#    print detectSpaces(ct,MSGS)








